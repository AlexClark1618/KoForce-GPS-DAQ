...
wdt = WDT(timeout=50000)  # 50 seconds

error_msg = (100, mac_id, 15, 0, time.ticks_ms(), ip_last_byte, 0, 0, 0, 0) # This tells me when the board restarted how long it took to reach the main loop since booting
packet = data_packing(packet_format, error_msg)
s.send(packet)
...
packet = data_packing(packet_format, info_msg)
s.send(packet)
...
#       timeValid = tup[5]
        ...
        send_msg = (inst, ID, RF, cal, ch, w_num, ms, sub_ms, event_num, count)
        try:
            packet = data_packing(packet_format, send_msg)
            data = s.send(packet)
            wdt.feed()
