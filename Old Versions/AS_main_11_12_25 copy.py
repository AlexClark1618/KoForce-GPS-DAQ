...
#s.setblocking(False)
#s = connect_socket(HOST,PORT)
 #SOme blocking may be good. I want the code to wait for data, but not to wait when sending. Maybe there is a good medium here.
s.settimeout(.05)
def send_data(d):
    global s
    while True:
...
while True:
    ...
    try:
        ...
        if not wlan.isconnected():
            ...
            con_to_wifi(ssid, password)
         #Make sure buffer isnt full before recieving
        
        req = recieve(20) #Dont need poller, can just check for socket activity on a not blocking socket
        ...
        if req:
            ...
            if inst == 99:
                ...
                for i in range(len(timesOfInterest)):
                    ...
                    print("5")
                    try:
                        send_data(packet) #Should I start checking wifi and socket are active before sending? It seems kind of pointless,
