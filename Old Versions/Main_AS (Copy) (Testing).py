...
s.setblocking(0.5) #SOme blocking may be good. I want the code to wait for data, but not to wait when sending. Maybe there is a good medium here.
...
def reconnect_socket(sock):
    try:
        ...
        #error_msg = (100, mac_id, 9, 0, 0, 0, 0, 0, 0, 0) #This doesnt make sense if there is an error its likely because of the socket so no way to send
        #packet = data_packing(send_packet_format, error_msg)
        #s.send(packet)
    time.sleep(1)
...
while True:
    print("1")
    sock_error = False #Socket Error flag
            ...
            if inst == 99:
                ...
                for i in range(len(timesOfInterest)):
                    ...
                    try:
                        ...
                        if e.args[0] != 110: #Not Blocking timeout
                            print("Send error:", e)
