...
def reconnect_socket(sock):
    try:
        ...
        error_msg = (100, mac_id, 9, 0, 0, 0, 0, 0, 0)
        packet = data_packing(send_packet_format, error_msg)
        s.send(packet)
    time.sleep(1)
...
def clear_wifi_rx_buffer(sock):      
    ...
    try:
        ...
        error_msg = (100, mac_id, 10, 0, 0, 0, 0, 0, 0)
        packet = data_packing(send_packet_format, error_msg)
        s.send(packet)
...
def maxRxBuf(n):
    try:
        ...
        error_msg = (100, mac_id, 2, 0, 0, 0, 0, 0, 0)
        packet = data_packing(send_packet_format, error_msg)
        s.send(packet)
...
error_msg = (100, mac_id, 15, time.ticks_ms(), 0, 0, 0, 0, 0) # This tells me when the board restarted how long it took to reach the main loop since booting
packet = data_packing(send_packet_format, error_msg)
s.send(packet)
...
while True:
    ...
    try:
        ...
        try:
            ...
            error_msg = (100, mac_id, 11, 0, 0, 0, 0, 0, 0)
            packet = data_packing(send_packet_format, error_msg)
            s.send(packet)
            reconnect_socket(s)
            continue
        ...
        if req:
            try:
                ...
                error_msg = (100, mac_id, 12, 0, 0, 0, 0, 0, 0)
                packet = data_packing(send_packet_format, error_msg)
                s.send(packet)
                continue
            ...
            if inst == 99:
                ...
                for i in range(len(timesOfInterest)):
                    ...
                    msg = (inst, ID, RF, cal, ch, w_num, ms, sub_ms, event_num)
                    ...
                    try:
                        ...
                        print("Send error:", e)
                        error_msg = (100, mac_id, 13, 0, 0, 0, 0, 0, 0)
                        packet = data_packing(send_packet_format, error_msg)
                        s.send(packet)
                        reconnect_socket(s)
